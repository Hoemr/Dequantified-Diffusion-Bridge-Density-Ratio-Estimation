...
@utils.register_model(name='toy_bridge_joint_scorenet')
class ToyBridgeJointScoreNetwork(nn.Module):
  """
  Simple MLP-based score network (for toy gaussian problems)
  """
...
@utils.register_model(name='toy_bridge_time_scorenet')
class ToyBridgeTimeScoreNetwork(nn.Module):
    """
    Simple MLP-based score network (for toy gaussian problems)
    """
    def __init__(self, config):
        ...
        self.net = nn.Sequential(
          nn.Linear(self.in_dim * 2 + 1, self.h_dim),
          nn.ELU(),
          nn.Linear(self.h_dim, self.h_dim),
          nn.ELU(),
          nn.Linear(self.h_dim, self.h_dim),
...
class MVNParamScoreNetwork(nn.Module):
  ...
  def forward(self, x, t):
    ...
    if self.config.model.bridge:
      # new_cov = id_mat - t * id_mat + t ** 2 * theta_mat
      # new_cov_inv = torch.inverse(new_cov)
      # new_cov_dt = -id_mat + 2*t*theta_mat
      new_cov = id_mat + t * (t - 1) * id_mat + t ** 2 * theta_mat
      new_cov_inv = torch.inverse(new_cov)
      new_cov_dt = (2*t - 1) * id_mat + 2*t * theta_mat
      
      x = x.view(-1, 1, 1, self.dim)
      term1 = torch.einsum('bii->b', (
          new_cov_dt @ new_cov_inv).squeeze()).unsqueeze(1)
      term2 = (x @ new_cov_inv @ new_cov_dt @ new_cov_inv @ x.permute(0, 1, 3, 2))[:, :, 0, 0]
      out_t = -0.5*term1 + 0.5*term2 
...
class ParamScoreNetwork(nn.Module):
  ...
  def forward(self, x, t):
    ...
    if self.config.model.bridge:
      coef = self.sigma_q**2
      denom = 1 - t + t**2 * coef
      out_t = (-1+2*t*coef)*((self.theta - x)**2 - denom)
      out_t = out_t/(2*denom**2)
