...
class ToyInterpXt(SDE):
  ...
  def marginal_prob_bridge(self, x0, xT, t):
    std = self.marginal_std_bridge(t)
    mean = (1 - t / self.T) * x0 + (t / self.T) * xT
    return mean, std
  ...
  def marginal_sample_brdige(self, x0, xT, t):
    mean, std = self.marginal_prob_bridge(x0, xT, t)
    return mean + std * torch.randn_like(x0)
