...
def train(config, workdir):
  ...
  """
  # Initialize model.
  score_model = mutils.create_model(config, name=config.model.name).to(torch.float64)
    ...
    if step % config.training.eval_freq == 0 and step > 0:
      if config.data.dataset != 'GaussiansforMI':
        if config.data.dataset == 'Toy2D':
          visualize_toy2d(config, train_ds, score_model, savefig=figures_dir, step=step, device=config.device)
        else:
...
def visualize_toy2d(config, dataset, model, savefig=None, step=None, device=None):
  model.eval()
  ...
  with torch.no_grad():
    ...
    # num_samples = 5000
    # ps, qs = dataset.sample_marginal(num_samples)
    ...
    x_points = np.linspace(xmin, xmax, 150)
    y_points = np.linspace(ymin, ymax, 150)
    X, Y = np.meshgrid(x_points, y_points)
    mesh = np.vstack([X.ravel(), Y.ravel()]).T  
    mesh = torch.from_numpy(mesh).to(device)
    
    # plot estimated ratios
    est_logr, _ = density_ratio_fn(model.to(device), mesh, score_type=config.model.type)
    est_logr = est_logr.reshape(X.shape)  # 将结果调整为网格形状
    ...
    im = ax.imshow(est_logr, origin="lower", extent=(xmin, xmax, ymin, ymax), cmap="viridis", aspect="auto")  # viridis     coolwarm
