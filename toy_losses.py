...
def toy_joint_score_estimation(scorenet, samples, t, eps=1e-5, likelihood_weighting=False, bridge=False, sde=None):
  """
  in objective, T = [0, 1]
  px, qx, xt: (batch_size, 1)
  t: (batch_size, 1)
  ...
  if likelihood_weighting:
    if not bridge:
      ...
      lambda_t = (t * (1 - t)).squeeze()
      lambda_t0 = (t0 * (1 - t0)).squeeze()
      lambda_t1 = ((t1 - eps) * (1 - t1 + eps)).squeeze()
      lambda_dt = (1 - 2 * t.squeeze())
  else:
    lambda_t = lambda_t0 = lambda_t1 = 1
    lambda_dt = 0
...
def toy_timewise_score_estimation(scorenet, samples, t, eps=1e-5, likelihood_weighting=False, bridge=False, sde=None):
  """
  in objective, T = [0, 1]
  px, qx, xt: (batch_size, 1)
  t: (batch_size, 1)
  ...
  if likelihood_weighting:
    if not bridge:
      ...
      # x0 is not considered into std (bridge)
      lambda_t = (t * (1 - t)).squeeze()
      lambda_t0 = (t0 * (1 - t0)).squeeze()
      lambda_t1 = ((t1 - eps) * (1 - t1 + eps)).squeeze()
      lambda_dt = (1 - 2 * t.squeeze())
      
      # # x0 is considered into std
      # lambda_t = (t * (1 - t)).squeeze()
      # lambda_t0 = (t0 * (1 - t0)).squeeze()
      # lambda_t1 = ((t1 - eps) * (1 - t1 + eps)).squeeze()
      # lambda_dt = (1 - 2 * t.squeeze())
  ...
  term4 = (2 * xt_score).squeeze() * lambda_dt
...
def get_step_fn(sde, train, joint=False, optimize_fn=None, reweight=False, bridge=False):
  ...
  def step_fn(state, batch, t):
    ...
    if train:
      ...
      if joint:
        ...
        loss = loss_fn(model, batch, t, likelihood_weighting=reweight, bridge=bridge, sde=sde)
      else:
        loss, loss1, loss2, loss3, edge1, edge2 = loss_fn(model, batch, t, likelihood_weighting=reweight, bridge=bridge, sde=sde)
      loss.backward()
