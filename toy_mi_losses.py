...
def toy_joint_score_estimation(sde, scorenet, qx, eps=1e-5, likelihood_weighting=False, bridge=False, config=None):
  """
  in objective, T = [0, 1]
  px, qx, xt: (batch_size, 1)
  t: (batch_size, 1)
  ...
  # xt = mean + px * std
  if bridge:
    xt = sde.marginal_sample_brdige(px, qx, t)
  else:
    xt = mean + px * std
  ...
  if not likelihood_weighting:
    ...
    if bridge:
      weighting_fn = lambda t: sde.marginal_std_bridge(t) ** 2
    else:
      weighting_fn = lambda t: sde.marginal_prob(torch.zeros_like(t), t)[1] ** 2
  ...
  # if torch.isnan(loss.mean().detach().cpu()):
  #   results = [score_x, score_t, ssm_loss1, ssm_loss2, term1, term2, term3, term4, lambda_dt, weighting_fn(t), t]
  #   results = [str(result.mean().detach().cpu().numpy().item()) for result in results]
  #   print("========================")
  #   print("||".join(results))
  #   raise ValueError("Loss is nan. Stopping training.")
  
  # print(loss.min().detach().cpu().numpy().item(), 
  #       loss.mean().detach().cpu().numpy().item(),
  #       loss.max().detach().cpu().numpy().item())
...
def toy_timewise_score_estimation(scorenet, samples, t, eps=1e-5, reweight=False, config=None):
  """
  in objective, T = [0, 1]
  px, qx, xt: (batch_size, 1)
  t: (batch_size, 1)
...
def get_step_fn(sde, train, joint=False, optimize_fn=None, reweight=False, bridge=False, config=None):
  ...
  def step_fn(state, batch):
    ...
    if train:
      ...
      if joint:
        ...
        loss = loss_fn(sde, model, batch, likelihood_weighting=reweight, bridge=bridge, config=config)
      else:
